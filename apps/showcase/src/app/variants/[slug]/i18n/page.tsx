...
export default async function I18nPage({
  ...
  const doc = await readVariantDoc(slug, "i18n.md");
  if (!v || !doc) notFound();

  return (
    <>
      <PageHeader
        kicker={`${v.meta.name} · i18n & RTL`}
        title="Internationalization & RTL"
        lede={`Recommended locales: ${v.i18n.recommendedLocales.join(", ")}. RTL: ${v.i18n.rtl.supported ? "supported" : "not supported"}.`}
